...
class VipsLoaderPolicyTest < ActiveSupport::TestCase
  ...
  test "loads AVIF" do
    skip "libvips built without HEIF support" unless heif_available?
  ...
  private
    # libvips only exposes the HEIF loader when built with libheif; some builds
    # (and ABI-mismatched module bundles) omit it, so guard the loader assertions.
    def heif_available?
      Vips.type_find("VipsOperation", "heifload").to_i > 0
    rescue Vips::Error
      false
    end
