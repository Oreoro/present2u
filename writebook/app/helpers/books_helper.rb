module BooksHelper
  # Technical features used across a deck's slides, with counts. Drives the
  # badges on the deck page.
  def deck_features(book)
    text = book.markable.to_s
    features = {
      latex: text.scan(/```(?:latex|tex|math|equation)\b/i).size,
      d2: text.scan(/```(?:d2|tala|d2-sketch|d2sketch)\b/i).size,
      math: text.scan(/\${1,2}[^$\n]+\${1,2}/).size,
      code: text.scan(/```([a-z0-9+#.-]+)/i).flatten.count { |lang| !%w[latex tex math equation d2 tala d2-sketch d2sketch].include?(lang.downcase) }
    }
    features.reject { |_, count| count.zero? }
  end
    ...
    if first_leaf = leaves.first
      link_to leafable_slug_path(first_leaf), data: hotkey_data_attributes("right"), class: "disable-when-arranging", hidden: true do
        tag.span(class: "btn") do
          image_tag("arrow-right.svg", aria: { hidden: true }, size: 24) + tag.span("Start presenting", class: "for-screen-reader")
  ...
  def link_to_previous_leafable(leaf, hotkey: true, for_edit: false)
    if previous_leaf = leaf.previous
      ...
      link_to book_slug_path(leaf.book), data: hotkey_data_attributes("left", enabled: hotkey), class: "btn" do
        image_tag("arrow-left.svg", aria: { hidden: true }, size: 24) + tag.span("Deck overview: #{ leaf.book.title }", class: "for-screen-reader")
      end
    end
  end
  ...
  def link_to_next_leafable(leaf, hotkey: true, for_edit: false)
    if next_leaf = leaf.next
      ...
      link_to book_slug_path(leaf.book), data: hotkey_data_attributes("right", enabled: hotkey), class: "btn txt-medium" do
        tag.span("Deck overview: #{leaf.book.title }", class: "overflow-ellipsis") + image_tag("arrow-reverse.svg", aria: { hidden: true }, size: 24)
      end
    end
  end
